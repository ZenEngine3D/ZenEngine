...
{	
	...
	void Viewport_PC::Initialize(HWND _hWndHandle, awVec2U16 _vSize)
	{
		mhMainWindow	= _hWndHandle;
		mvSize			= _vSize;
	}
	*/
	Window::Window(const WCHAR* _zWindowName, awVec2U16 _ClientSize)
	...
	LRESULT CALLBACK Window::WndEventCallback(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SIZING:	{ 
			...
			pParentWindow->SetSize(awVec2U16(awU16(pSize->right-pSize->left), awU16(pSize->bottom-pSize->top)));
			break; }
		case WM_PAINT:	{
				PAINTSTRUCT ps;

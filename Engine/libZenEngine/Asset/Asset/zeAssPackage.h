...
namespace zen { namespace zeAss
{
	...
	public:		
		...
		ZENInline const zWString&					GetStorageName	()const;
		...
		bool										Init			(zU32 _uID, const char* _zName, const char* _zGroup, const zChar* _zStorageName, zU32 _uEngineVer);
		ZENInline void								AssetAdd		(zeAss::Asset& _Asset);
		ZENInline void								AssetRem		(zeAss::Asset& _Asset);
		
		ZENInline void								SetDirty		(bool _bDirty);
		ZENInline bool								IsDirty			()const;
	protected:		
		...
		zWString									mzStorageName;		//!< Where to save this package(filename, database entry id, etc....)

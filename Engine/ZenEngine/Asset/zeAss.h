#pragma once

//=================================================================================================
//! @file		FrameworkAssetAll.h
//! @brief		AllocHeader file to include all sub-header files of Asset library
//! @details	Included by platforms that can load offline asset for edition and export
//-------------------------------------------------------------------------------------------------
//!	@addtogroup LibAsset
//!				Asset edition and export of the engine. 
//! @n@n		Made of many components...
//!				-# @ref LibAsset-Gfx @copydoc LibAsset-Gfx
//-------------------------------------------------------------------------------------------------
//! @mainpage	Zen Engine
//! @section	mainpage-libasset Section Asset Library
//=================================================================================================

#include "Asset/zeAssItem.h"
#include "Asset/zeAssPackage.h"
#include "Asset/zeAssPackageGroup.h"
#include "Asset/zeAssManager.h"
#include "Asset/zeAssLoader.h"
#include "Graphic/zeAssGfxMesh.h"
#include "Graphic/zeAssGfxTexture2D.h"

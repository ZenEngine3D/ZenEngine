...
namespace EExp
{
	...
	bool SerialGfxShaderBinding_DX11::ExportStart()
	{
		...
		pExportInfo->mdStagePerParamDef.Export( *(awArrayStatic<awHash64>*)&mSerial.maParamDefID, maStagePerParamDef);

		//---------------------------------------------------------------------
		// Process texture binding infos for each shader stage
		//---------------------------------------------------------------------
		Hashmap<TextureSlot>::Key32 dTextureBindInfo(16);
		dTextureBindInfo.SetDefaultValue( TextureSlot() );
		for( awUInt stageIdx(0), stageCount(mSerial.maShaderID.Count()); stageIdx<stageCount; ++stageIdx)
		{
			EExp::SerialShader_DX11* pSerialShader = EMgr::SerialItems.GetItem<EExp::SerialShader_DX11>( mSerial.maShaderID[stageIdx] );
			if( pSerialShader )
			{				
				for(awUInt idxTexShader=0; idxTexShader < pSerialShader->maTextureSamplerSlot.Count(); ++idxTexShader)
				{						
					SerialShader_DX11::BindInfo& SlotInfoIn	= pSerialShader->maTextureSamplerSlot[idxTexShader];
 				 	awHash32 hTextureName						= pSerialShader->maTextureSamplerName[idxTexShader];					
		...
		//---------------------------------------------------------------------	
		Hashmap<awU32>::Key32	dParamDefPerName(64);	
		dParamDefPerName.SetDefaultValue(0);
		for(awUInt paramDefIdx(0), paramDefCount(mSerial.maParamDefID.Count()); paramDefIdx<paramDefCount; ++paramDefIdx )
		{

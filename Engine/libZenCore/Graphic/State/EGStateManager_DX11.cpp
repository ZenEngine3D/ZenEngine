...
{	
	...
	for(awU8 uBlendMask=0; uBlendMask<16; ++uBlendMask)
	{
...
void ManagerState_DX11::SetBlend(const ConfigBlend& _Blend, awUInt _uRenderTarget)
{
...
void ManagerState_DX11::PipelineUpdate()
{
	...
	if( mbUpdatedView )
	{
		...
		{
			// Setup rendertarget
			awUInt targetCount = mrView->Get().marTargetColor.Count();
			for( awUInt targetIdx(0); targetIdx<kuMaxRenderTarget; ++targetIdx)
			{

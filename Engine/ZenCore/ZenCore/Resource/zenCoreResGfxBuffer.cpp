...
zU8* zGfxVertex::Lock()
{
	...
	zcRes::zGfxVertex* pVertex = static_cast<zcRes::zGfxVertex*>(mpResource);
	return pVertex->Lock();
}
...
{
	ZENAssertMsg(mpResource, "No valid resource assigned");
	zcRes::zGfxIndex* pIndex = static_cast<zcRes::zGfxIndex*>(mpResource);
	return pIndex->Lock();
}
...
{
	ZENAssertMsg(mpResource, "No valid resource assigned");
	zcRes::zGfxTexture2D* pTexture = static_cast<zcRes::zGfxTexture2D*>(mpResource);
...
{
	ZENAssertMsg(mpResource, "No valid resource assigned");
	zcRes::zGfxRenderTarget* pRenderTarget = static_cast<zcRes::zGfxRenderTarget*>(mpResource);

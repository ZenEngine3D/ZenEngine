...
bool SerialShader_DX11::ExportWorkCompile()
{
	...
	const char* pzShaderStage[zcExp::keShaderStage__Count]={"vs_5_0","ps_5_0"};	//Must match eShaderStage enum
...
{
	ExportInfo*								pExportInfo = static_cast<ExportInfo*>(mpExportInfo);
	BindInfo								aTextureBind[zcExp::kuDX11_TexturePerStageMax];
	...
	maParamDefID.SetCount(zcExp::keShaderParamFreq__Count);
	if( SUCCEEDED( D3DReflect( mSerialCommon.maCompiledShader.First(), mSerialCommon.maCompiledShader.Size(), IID_ID3D11ShaderReflection, (void**) &pGfxShaderReflection ) ) )
	{
		...
		for( zUInt uResIdx=0; uResIdx<shaderDesc.BoundResources; ++uResIdx )
		{
			...
			{				
				...
				{
					ZENAssert( uTextureCount < zcExp::kuDX11_TexturePerStageMax );
	...
	if( mpExportInfo->IsSuccess() && Super::ExportEnd() )
	{
		for(zUInt idx=0; idx<maParamDefID.Count(); ++idx)
		{
			if( maParamDefID[idx].IsValid() )
				maParamDefID[idx] = zcExp::CreateGfxShaderParamDef(mResID, static_cast<eShaderParamFreq>(idx));
		}
		return true;
	}	
	return false;
...
bool SerialGfxShaderVertex_DX11::ExportEnd()
{
	if( mpExportInfo->IsSuccess() && Super::ExportEnd() )
	{		
		mSerial.mShaderInputSignatureID	= zcExp::CreateGfxInputSignature( mResID );
		return TRUE;
	}				
...
bool SerialGfxShaderVertex_DX11::Serialize( zcExp::Serializer_Base& _Serializer )
{
	return FALSE; 
}

bool SerialGfxShaderPixel_DX11::Serialize( zcExp::Serializer_Base& _Serializer )
{
	return FALSE; 
}

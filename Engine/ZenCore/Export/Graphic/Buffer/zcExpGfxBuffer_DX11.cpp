...
{		
	...
	if( (pExportInfo->muElementSize % 16) == 0 )
	{
		mrExportData->muElementSize		= pExportInfo->muElementSize;
		mrExportData->muElementCount	= zenMath::Max<zU32>(pExportInfo->muElementCount, pExportInfo->maData.Count()/pExportInfo->muElementSize );
	...
	return false; //! @todo error message about element size
}
